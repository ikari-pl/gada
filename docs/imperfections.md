...
### String-keyed and string-valued maps deferred to Phase 4
*Where:* `compiler/internal/emit/emit.go` — `mapKeyBaseName` and `mapValueBaseName` reject `*ir.StringType` with explicit "await Phase 4" errors; the IR + translate layers accept `map[string]int` and `map[int]string` cleanly. The runtime's `Gada.Core.Maps` generic instantiates `Key_Type` and `Value_Type` via `is private` formals, which require *definite* types — Ada's `String` is unconstrained and so cannot stand in directly.
*Why not fixed yet:* two separate pieces of infrastructure are missing. (1) String hashing needs `Gada.Core.Hash.Hash_String` with SipHash-1-3 (DoS-resistant matching Go's `runtime.strhash`) — the Knuth Fibonacci-multiplicative used for `Hash_Integer` is fine for keys with high entropy but unsafe against adversarial collision inputs. (2) Definite-type wrapping needs an `Unbounded_String`-or-equivalent shim so map cells can store variable-length strings inline; the Maps runtime's per-cell storage assumes fixed-size cell records. Both ride along with Phase 4 (Stdlib wave 2 — strings package).
*What "fixed" looks like:* a fixture `map_string_int.go` adds to the corpus; emit selects `Hash => Gada.Core.Hash.Hash_String` for string keys; the runtime instantiates over `Unbounded_String` (or a thin wrapper) so cells stay definite-sized; round-trip from `m["key"] = 42` through to `Get` returns `42` with no allocation per lookup beyond the libgc-traced backing.
*Tracker:* `roadmap/05-stdlib-wave1.md` strings package; SipHash-1-3 implementation is on the runtime side, not the compiler side.

### Phase 2 panic payload type is fixed at Integer
*Where:* `compiler/internal/emit/emit.go` — `emitPanicInstantiation` writes a single `package Panic_Of_Integer is new Gada.Core.Panic (Payload_Type => Integer, Default => 0);` per program. `panic` of anything other than an int literal/expression compiles silently to `Panic_Of_Integer.Do_Panic (V);` where V's typing isn't checked, and `panic("string")` would emit garbage.
*Why not fixed yet:* Go's `panic(v)` accepts `any` (`interface{}`); the runtime side needs the `Gada.Reflect.Any` discriminated record (Phase 4) to land first so the compiler-emit can pick `Payload_Type => Gada.Reflect.Any` per program. The shape is also driven by what Phase 3's per-task panic-state TLS slot needs — that work refines whether the instantiation lives at package level or inside the per-program scheduler-init unit.
*What "fixed" looks like:* `Gada.Reflect.Any` ships with the obvious constructors (`Of_Integer`, `Of_Float`, `Of_String`, …); emit-side picks `Payload_Type => Gada.Reflect.Any` and lowers `panic(x)` to `Panic_Of_Any.Do_Panic (Of_<Type> (X));`. Recover round-trips through the matching `As_<Type>` accessor with a Constraint_Error on type mismatch (Go's pattern is `r, ok := r.(string)` — the `ok` half lifts as the discriminant check).
*Tracker:* `roadmap/04-interfaces-reflection.md` — `Gada.Reflect.Any` is the load-bearing piece; this lift is its first downstream consumer.
...
### TSan-built runtime not yet wired for the multi-worker scheduler
*Where:* `runtime/gada_core.gpr` C_Switches and the GNAT FSF 15.1 toolchain shipped via Alire.
*Why not fixed yet:* The Phase 3 sub-item 3b verify gate is "100-run no flake + TSan-clean run". The 100-run gate landed (zero failures on the dev host); the TSan side is gated on having a GNAT runtime built with `-fsanitize=thread`. GNAT FSF 15.1 from Alire ships only the default runtime, and rebuilding it with TSan is a multi-hour from-source compile against gcc. We shipped 3b on the 100-run zero-flake gate alone — the libco-side fix (`-DLIBCO_MP -Dthread_local=__thread`) is small and well-isolated, and the Ada side runs through protected objects + TLS that aren't on TSan's interesting paths.
*What "fixed" looks like:* one of (a) Alire ships a TSan-instrumented gnat_native crate, (b) a Phase 3 follow-up adds a `make tsan` target that uses a `--build-from-source` GNAT runtime, or (c) sub-item (c) work-stealing's verification builds TSan once and pins the result. The multi-worker test asserts goroutines distribute across workers; under TSan the same test would also assert no data races. Not load-bearing for correctness — the 100-run gate is the functional bar — but worth re-covering when (c) lands.
*Tracker:* roadmap [03-concurrency.md](../roadmap/03-concurrency.md) sub-item 3c follow-up.
