...
- [ ] **GADA.Async.Scheduler — M:N over a fixed pool of Ada tasks**
  ...
  - [ ] **(b) GOMAXPROCS worker pool**
        ...
        *Prereq B — pending, blocks (b):* Replace `Ada.Task_Attributes` (the current "current goroutine" lookup) with per-worker storage that doesn't cross a process-global hash. The first multi-worker attempt surfaced an intermittent crash whose top suspect is Task_Attributes thread-safety under concurrent `Set_Value` from N worker tasks (RM C.7.2 doesn't require it to be task-safe; GNAT FSF uses a global keyed-on-Task_Id hash that races). Full investigation: [[docs/incidents/2026-05-03-scheduler-3b-multi-worker-race]]. The next attempt MUST land Prereq B and the worker-pool extension as one atomic commit — no half-shipped multi-worker code on top of the broken Task_Attributes path.
        *Verify:* `make -C runtime test PKG=async.scheduler` — multi-spawn test schedules N goroutines and confirms they ran on at least 2 distinct worker tasks; runs 100x with no flakes (`for i in {1..100}; do runtime/tests/obj/test_runner async.scheduler || break; done`); TSan-built runtime under the same multi-worker test reports zero data races.
        *Done when:* Pool of `Worker` tasks sized by `System.Multiprocessors.Number_Of_CPUs` (overridable via `Init (Workers => N)`). All workers share the same protected-object FIFO queue (work-stealing arrives in (c)). Workers exit cleanly when `Shutdown` signals the queue is permanently drained. The "current goroutine" lookup uses per-worker storage (Prereq B), not `Ada.Task_Attributes`.
