...
func (e *emitter) collectSliceElems() {
	...
	for _, d := range e.file.Decls {
		if td, ok := d.(*ir.TypeDecl); ok {
			...
			// composite literal against the declared field set, and
			// interface names so typeName renders a NamedType referring
			// to one as `Name'Class`.
			switch u := td.Underlying.(type) {
			case *ir.StructType:
...
func (e *emitter) emitMainProcedure() {
	...
	hasTypes := e.fileHasTypeDecls()
	hasDeclSection := hasTypes || hasSlices || hasMaps || hasChans || hasSelectors || hasPanic || len(others) > 0 || len(mainDecls) > 0 || (len(mainDefers) > 0 && !mainDefersInsideWrapper) || len(mainGos) > 0
	...
	if hasTypes && (hasSlices || hasMaps || hasChans || hasSelectors || hasPanic || len(others) > 0 || len(mainDecls) > 0 || (len(mainDefers) > 0 && !mainDefersInsideWrapper) || len(mainGos) > 0) {
		e.println("")
	}
	if hasSlices {
...
func (e *emitter) emitPackageBody() {
	...
	if hasTypes || hasSlices || hasMaps || hasChans || hasSelectors || hasPanic || len(fns) > 0 {
		e.println("")
	}
	...
	if hasTypes && (hasSlices || hasMaps || hasChans || hasSelectors || hasPanic) {
		e.println("")
	}
	if hasSlices {
		e.emitSliceInstantiations()
	...
	if hasTypes || hasSlices || hasMaps || hasChans || hasSelectors || len(fns) > 0 {
		e.println("")
	}
...
func (e *emitter) subpHeader(fn *ir.Function) (string, string, bool) {
	...
	if fn.Receiver != nil {
		...
		header, err := e.dispatchOpSpec("overriding ", recv, adaIdent(fn.Receiver.Type), fn.Name, fn.Params, fn.Results, " is")
		if err != nil {
			e.fail(err)
			return "", name, false
		}
	...
	rt, err := e.typeName(fn.Results[0].Type)
	if err != nil {
		e.fail(err)
		return "", name, false
	}
...
// typeName renders an IR type as its Ada type name. It is a method
// because a NamedType resolves against the file's declarations: a name
// that denotes an interface becomes the class-wide `Name'Class` view
// (RM 3.9.2 — a call on a class-wide operand dispatches), a struct name
// becomes the plain record type, and a name that resolves to neither is
// a loud error.
func (e *emitter) typeName(t ir.Type) (string, error) {
	switch t := t.(type) {
	case *ir.IntType:
		return "Integer", nil
	case *ir.StringType:
	...
	case *ir.Float64Type:
		return "Long_Float", nil
	case *ir.NamedType:
		if e.interfaceNames[t.Name] {
			return adaIdent(t.Name) + "'Class", nil
		}
